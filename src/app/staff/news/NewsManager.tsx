      ...
      {workspace === "page" ? (
        <div className="news-builder-grid">
          <section className="news-admin-panel news-builder-panel">
            <div className="news-admin-panel-heading"><div><span className="staff-kicker">Page settings</span><h2>Hero & presentation</h2><p>Use the text-first Operations bulletin for the public newsroom, or select a custom banner image when one is specifically needed.</p></div></div>
            <div className="news-builder-form">
              <label><span>Hero layout</span><select disabled={!canEdit} value={pageSettings.heroMode} onChange={(event) => setPageSettings({ ...pageSettings, heroMode: event.target.value as NewsPageSettings["heroMode"] })}><option value="editable">Operations bulletin (recommended)</option><option value="image">Custom banner image</option></select></label>
              {pageSettings.heroMode === "image" ? <><label className="wide"><span>Hero image</span><div className="news-builder-upload-row"><input disabled={!canEdit} value={pageSettings.heroImage} onChange={(event) => setPageSettings({ ...pageSettings, heroImage: event.target.value })} /><label className={`news-admin-primary news-upload-button ${!canEdit ? "disabled" : ""}`}>Upload image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" disabled={!canEdit || busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadImage(file, (url) => setPageSettings((current) => ({ ...current, heroImage: url }))); event.currentTarget.value = ""; }} /></label></div></label><label><span>Image position</span><input disabled={!canEdit} value={pageSettings.heroImagePosition} onChange={(event) => setPageSettings({ ...pageSettings, heroImagePosition: event.target.value })} placeholder="center center" /></label></> : null}
            ...
            {pageSettings.heroMode === "image" && pageSettings.heroImage ? <div className="news-builder-preview"><img src={pageSettings.heroImage} alt="Hero preview" style={{ objectPosition: pageSettings.heroImagePosition }} /></div> : null}
          </section>
