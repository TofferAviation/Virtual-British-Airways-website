...
export default async function AccountPage() {
  ...
  return (
    <main className="account-v2-page">
      ...
      <nav className="account-v2-tabs" aria-label="Account sections"><div className="account-v2-container account-v2-tabs-inner"><a className="active" href="#trips">Your trips</a><Link href="/account/profile">Account settings</Link><a href="#membership">Membership</a><Link href="/support/tickets">Support tickets</Link></div></nav>
      ...
      <section className="account-v2-main" id="trips"><div className="account-v2-container"><h2>Your pilot dashboard</h2><p className="account-v2-subtitle">Your career data is maintained directly by British Airways Virtual.</p><div className="account-v2-stat-grid">{stats.map((stat) => <article className="account-v2-stat" key={stat.label}><span className="account-v2-stat-label">{stat.label}</span><strong className="account-v2-stat-value">{stat.value}</strong><small className="account-v2-stat-note">{stat.note}</small></article>)}</div><div className="account-v2-lower">
        <article className="account-v2-card account-v2-flights"><h3>Recent flight reports</h3><div className="account-v2-flight-list">{pireps.length ? pireps.slice(0, 5).map((pirep) => <div className="account-v2-flight-row" key={pirep.id}><strong>{pirep.flightNumber}</strong><span>{pirep.from} → {pirep.to}</span><span>{pirep.aircraft}</span><span>{Math.floor(pirep.blockMinutes / 60)}h {pirep.blockMinutes % 60}m</span><span>{pirep.landingFpm == null ? "—" : `${pirep.landingFpm} fpm`}</span><span className={`ops-pirep-chip ${pirep.status}`}>{pirep.status.replaceAll("_", " ")}</span></div>) : <p className="account-v2-subtitle">No BAV PIREPs yet. Manual reports and future FreeFlight ACARS flights will appear here.</p>}</div></article>
        <aside className="account-v2-card account-v2-assignment"><h3>Active assignment</h3>{assignment ? <><strong>{assignment.flightNumber} · {assignment.from} → {assignment.to}</strong><p>{assignment.date} · {assignment.aircraft} · {assignment.departure}</p>{flightPlan ? <p><b>Flight plan:</b> {flightPlan.status === "synced" ? `SimBrief OFP ${flightPlan.simbriefOfpId ? `#${flightPlan.simbriefOfpId}` : "synced"}` : "Briefing not yet synced"}</p> : null}<Link className="ops-inline-link" href="/manage-assignment">Manage assignment & view briefing →</Link><Link className="ops-inline-link" href="/operations/pirep">Submit manual PIREP →</Link></> : <><strong>No flight booked</strong><p>Choose a flight from the BAV schedule when you&apos;re ready to fly.</p></>}<Link href="/book">Find a virtual flight →</Link><div className="account-v2-tier-box" id="membership"><div className="account-v2-tier-head"><span>Next rank / tier</span><span>{account.tierPoints.toLocaleString()} / {tierTarget.toLocaleString()}</span></div><div className="account-v2-progress" aria-label={`${tierProgress.toFixed(0)} percent progress toward next tier`}><span style={{ width: `${tierProgress}%` }} /></div><p>Tier progression is calculated from accepted BAV flight records.</p></div></aside>
      </div></div></section>

      <footer className="account-v2-footer"><div className="account-v2-footer-inner"><div className="account-v2-footer-brand"><BrandLogo variant="white" /></div><div><h4>About</h4><Link href="/about">About the VA</Link><Link href="/fleet">Fleet</Link><Link href="/destinations">Destinations</Link></div><div><h4>Support</h4><Link href="/help">Help</Link><Link href="/support/tickets">Support tickets</Link><Link href="/service-status">Service status</Link></div><div><h4>Account</h4><Link href="/account/profile">Account settings</Link><Link href="/book">Flights</Link><Link href="/operations/pirep">Submit PIREP</Link></div></div></footer>
    </main>
  );
}
