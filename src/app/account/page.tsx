...
export default async function AccountPage() {
  ...
  const firstFlightAward = account.awards.find((award) => award.id === "first-flight");
  const stats = [
  ...
  return (
    <main className="account-v2-page">
      ...
      <section className="account-v2-main" id="trips"><div className="account-v2-container"><h2>Your pilot dashboard</h2><p className="account-v2-subtitle">Your career data is maintained directly by British Airways Virtual.</p><div className="account-v2-stat-grid">{stats.map((stat) => <article className="account-v2-stat" key={stat.label}><span className="account-v2-stat-label">{stat.label}</span><strong className={`account-v2-stat-value${stat.label === "CURRENT RANK" ? " account-v2-rank-value" : ""}`}>{stat.label === "CURRENT RANK" ? <><RankInsignia rank={account.rank} size="compact" /><span>{stat.value}</span></> : stat.value}</strong><small className="account-v2-stat-note">{stat.note}</small></article>)}</div>{firstFlightAward ? <article className="account-v2-award"><span className="account-v2-award-mark" aria-hidden="true">★</span><div><span className="account-v2-award-kicker">CAREER AWARD</span><strong>First Flight</strong><p>Earned {new Date(firstFlightAward.awardedAt).toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" })} after your first accepted BAV PIREP.</p></div></article> : null}<div className="account-v2-lower">
